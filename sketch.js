...
function generateQuestions() {
    ...
    for (let i = 0; i < 10; i++) {
        // ALLEEN Lineaire vergelijkingen: ax + b = cx + d
        // Met verschillende volgordes voor variatie
        
        let a = floor(random(2, 10));   // Coëfficiënt links
        ...
        // VARIATIE: Verschillende volgordes (random gekozen)
        let variant = floor(random(4));  // 4 verschillende stijlen
        let text = "";
        
        if (variant === 0) {
            // Stijl 1: ax + b = cx + d  (origineel)
            let leftSide = (a === 1 ? "x" : a + "x") + (b >= 0 ? " + " + b : " - " + Math.abs(b));
            let rightSide = "";
            ...
            // Stijl 2: b + ax = d + cx  (constante eerst, ZONDER haakjes)
            let leftSide = "";
            if (b >= 0) {
                leftSide = b + (a === 1 ? " + x" : " + " + a + "x");
            } else {
                leftSide = b + (a === 1 ? " + x" : " + " + a + "x");
            }
            ...
            if (c === 0) {
                ...
                if (d >= 0) {
                    rightSide = d + (c === 1 ? " + x" : (c === -1 ? " - x" : (c > 0 ? " + " + c + "x" : " - " + Math.abs(c) + "x")));
                } else {
                    rightSide = d + (c === 1 ? " + x" : (c === -1 ? " - x" : (c > 0 ? " + " + c + "x" : " - " + Math.abs(c) + "x")));
                }
            }
            ...
            // Stijl 3: ax + b = d - cx  (min teken rechts)
            let leftSide = (a === 1 ? "x" : a + "x") + (b >= 0 ? " + " + b : " - " + Math.abs(b));
            let rightSide = "";
            ...
            if (c === 0) {
                ...
                // Draai teken om: d - cx in plaats van d + (-c)x
                rightSide = d + (c === 1 ? " - x" : " - " + c + "x");
            } else {
                // c is negatief, dus wordt positief
                rightSide = d + (c === -1 ? " + x" : " + " + Math.abs(c) + "x");
            }
            
            text = leftSide + " = " + rightSide;
            
        } else {
            // Stijl 4: b + ax = d - cx  (beide anders, ZONDER haakjes)
            let leftSide = "";
            if (b >= 0) {
                leftSide = b + (a === 1 ? " + x" : " + " + a + "x");
            } else {
