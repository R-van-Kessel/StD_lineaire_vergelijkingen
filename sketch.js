...
function generateQuestions() {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        // Oplossen: ax - cx = d - b → x = (d - b) / (a - c)
        
        let a = floor(random(2, 10));   // Coëfficiënt links, Grotere getallen = moeilijker
        let b = floor(random(-20, 20)); // Constante links, - Groter bereik = meer variatie
        let c = floor(random(-5, 5));   // Coëfficiënt rechts (kan negatief)
        let d = floor(random(-20, 20)); // Constante rechts
        
        // Zorg dat a ≠ c (anders oneindig of geen oplossingen)
        if (a === c) c = c + 1;
        
        // Bereken antwoord: x = (d - b) / (a - c)
        let answer = Math.round((d - b) / (a - c));
        
        // Controleer of antwoord een heel getal is, anders opnieuw
        if (!Number.isInteger(answer) || Math.abs(answer) > 20) { //Verander 20 naar groter getal voor grotere antwoorden
            i--; // Probeer deze vraag opnieuw
            continue;
        }
        
        // Maak de vergelijking tekst
