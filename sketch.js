...
function generateQuestions() {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        // Zorg dat a ≠ c (anders geen unieke oplossing)
        if (a === c) c = c + 1;
        
        // Bereken antwoord: ax + b = cx + d
        // ax - cx = d - b
        // x(a - c) = d - b
        // x = (d - b) / (a - c)
        let answer = (d - b) / (a - c);
        
        // Controleer of antwoord een geldig heel getal is
        if (!Number.isInteger(answer) || Math.abs(answer) > 20) {
            i--;
            continue;
        }
        
        // VARIATIE: Verschillende schrijfwijzen
        let variant = floor(random(4));
        let text = "";
        
        if (variant === 0) {
            // Stijl 1: ax + b = cx + d
            let leftSide = (a === 1 ? "x" : a + "x") + (b >= 0 ? " + " + b : " " + b);
            let rightSide = "";
            ...
            let leftSide = b + (a === 1 ? " + x" : (a > 0 ? " + " + a + "x" : " " + a + "x"));
            let rightSide = "";
            ...
            // Stijl 3: ax + b = d - cx (minteken rechts)
            let leftSide = (a === 1 ? "x" : a + "x") + (b >= 0 ? " + " + b : " " + b);
            let rightSide = "";
