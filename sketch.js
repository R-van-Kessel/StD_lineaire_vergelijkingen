...
function generateQuestions() {
    blocks = [];                    // Maak lege lijst voor alle blokjes
    isChecked = false;             // Nog niet nagekeken
    correctCount = 0;              // Score op 0
    isFlashing = false;            // Geen flits effect
    flashCounter = 0;              // Flits teller op 0

    // Update de score knop naar 0/10
    if (scoreButton && scoreButton.elt) scoreButton.elt.textContent = 'Score: 0/10';
    scoreButton.style('background-color', '#9C27B0');

    let questions = [];            // Lijst voor vragen
    let answers = [];              // Lijst voor antwoorden

    for (let i = 0; i < 10; i++) {
    // Lineaire vergelijkingen - altijd in vorm: ax + b = cx + d
    
    let a = floor(random(2, 10));   // Coëfficiënt links (nooit 0)
    let b = floor(random(-20, 20)); // Constante links
    let c = floor(random(-5, 5));   // Coëfficiënt rechts
    let d = floor(random(-20, 20)); // Constante rechts
    
    // Zorg dat a ≠ c
    if (a === c) c = c + 1;
    
    // Zorg dat b en d NOOIT 0 zijn
    if (b === 0) b = floor(random(1, 10)) * (random() > 0.5 ? 1 : -1);
    if (d === 0) d = floor(random(1, 10)) * (random() > 0.5 ? 1 : -1);
    
    // c mag WEL 0 zijn, maar dan moet de vergelijking anders
    
    // Bereken antwoord: x = (d - b) / (a - c)
    let answer = (d - b) / (a - c);
    
    // Controleer of antwoord geldig is
    if (!Number.isInteger(answer) || Math.abs(answer) > 20) {
        i--;
        continue;
    }
    
    // ALLEEN variant 0 en 1
    let variant = floor(random(2));
    let text = "";
    ...
    if (variant === 0) {
        ...
        let rightSide = "";
        if (c === 0) {
            // Als c=0, gewoon d tonen (geen x term)
        ...
        // Stijl 2: b + ax = d + cx (constante eerst)
        let leftSide = b.toString();
        if (a === 1) {
            leftSide += " + x";
        } else if (a > 0) {
            leftSide += " + " + a + "x";
        } else {
            leftSide += " - " + Math.abs(a) + "x";
        }
        
        let rightSide = d.toString();
        if (c === 0) {
            // Als c=0, alleen d tonen
            rightSide = d.toString();
        } else if (c === 1) {
            rightSide += " + x";
        } else if (c === -1) {
    ...
    answers = shuffle(answers);    // Schud de antwoorden door elkaar

    // MAAK VRAAG BLOKJES (rij 0 en 1):
    let questionIndex = 0;
    for (let row = 0; row < 2; row++) {        // Rij 0 en 1
        for (let col = 0; col < COLS; col++) { // Alle kolommen
            blocks.push({
            col: col,
            row: row,
            startCol: col,          // ← MOET ER ZIJN
            startRow: row,          // ← MOET ER ZIJN
            x: MARGIN + col * CELL_SIZE,
            y: MARGIN + row * CELL_SIZE + BUTTON_HEIGHT + TITLE_SPACE,
            isDragging: false,      // ← MOET ER ZIJN
            isPlaced: false,        // ← MOET ER ZIJN
            text: questions[questionIndex].text,
            answer: questions[questionIndex].answer,
            isQuestion: true,
            isCorrect: null         // ← MOET ER ZIJN
        });
        questionIndex++;
    }
}
    // MAAK ANTWOORD BLOKJES (rij 2 en 3):
    let answerIndex = 0;
    for (let row = 2; row < 4; row++) {        // Rij 2 en 3
        for (let col = 0; col < COLS; col++) { // Alle kolommen
            blocks.push({
            col: col,
            row: row,
...
function drawBlock(block) {
    ...
    noStroke();
    textAlign(CENTER, CENTER);
    textSize(11);  // ← VERANDER DIT GETAL!
    textStyle(BOLD);
